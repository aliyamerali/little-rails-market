...
RSpec.describe BulkDiscount do
  ...
  describe 'instance methods' do
    ...
    it '#delete_valid? returns true if a discount *is not* applied to any invoices with status in-progress, else false' do
      ...
      expect(discount_1.delete_valid?).to eq(true)
      expect(discount_2.delete_valid?).to eq(false)
    end

    it '#create_valid? returns false if discount will always be superceded by existing discount' do
      merchant = Merchant.create!(name: 'Sal\'s Signs')
      discount_1 = merchant.bulk_discounts.create!(percentage: 10.0, quantity_threshold: 10)

      discount_2 = merchant.bulk_discounts.new(percentage: 10.0, quantity_threshold: 10)
      expect(discount_2.create_valid?).to eq(false)

      discount_3 = merchant.bulk_discounts.new(percentage: 15.0, quantity_threshold: 12)
      expect(discount_3.create_valid?).to eq(true)

      discount_4 = merchant.bulk_discounts.new(percentage: 15.0, quantity_threshold: 5)
      expect(discount_4.create_valid?).to eq(true)
    end
  end

  it '#update_valid? returns true if a discount *is not* applied to any invoices with status in-progress AND terms will not be superceded by existing discount' do
    merchant = Merchant.create!(name: 'Sal\'s Signs')
    discount_1 = merchant.bulk_discounts.create!(percentage: 10.0, quantity_threshold: 10)
    discount_2 = merchant.bulk_discounts.create!(percentage: 5.0, quantity_threshold: 5)
    ...
    discount_3 = merchant.bulk_discounts.new(percentage: 10.0, quantity_threshold: 10)
    expect(discount_3.update_valid?).to eq(false)
  end
end
